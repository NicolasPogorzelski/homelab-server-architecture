...
- **Next session (queued for 2026-06-09):** revisit the calibre auto-import config — weigh pros/cons of the current local-copy-on-CIFS approach vs. the alternatives in [ADR calibre-cifs-sqlite-import](docs/decisions/calibre-cifs-sqlite-import.md) (esp. `nobrl` mount option vs. moving `metadata.db`/library to local-block storage), and adjust the config if warranted. Feature is live + verified; this is an optimization/architecture review, not a fix.
