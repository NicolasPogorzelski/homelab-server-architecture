...
- **Retro gaming — cross-device saves (planned, next session):** sync `.srm` (in-game) + `.state` (save states) across clients via a **separate writable** Samba share `[saves]` (do NOT make `[roms]` writable). RetroArch `savefile_directory` + `savestate_directory` → mounted saves path. Caveats: save states are core-version-sensitive; no simultaneous play (last-write-wins). Open design question: reuse `roms` user with RW on `[saves]` vs. new `saves` user; one shared folder vs. per-client subfolders.
