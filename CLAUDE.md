...
- **MergerFS pool ~99.5% full on vm102 (by design; alert tiering done 2026-07-10):** the media
  ...
  wrong measure for a multi-terabyte archive (15% is several TB). The rule now excludes `cifs`, `fuse.mergerfs`
  and the member disks; the new `ArchivePoolLowSpace` warns on **absolute** free bytes below
  100 GiB with `for: 1h`. Currently a low-hundreds-of-GiB free.
