...
| Date | Change |
|---|---|
| 2026-05-29 | VM102 Samba: `access based share enum = yes` added to global (replaces `browseable = no`); `[media]` share updated — `storage` added to `valid users` + `write list` (primary media management user); `browseable = no` removed from `[media]` |
| 2026-05-29 | media stack documented: `tag:storage-client` added to Tailscale ACL (Rule 8: storage:445 + media-client:55435); `[media]` share on VM102 with two-user Samba model (`data-admin` RW, `media` RO); `docs/services/storage-stack.md` created; `samba.md`, `vm102.md`, `tailscale-acl.md` updated |
...
| 2026-04-30 | Docker engine data root migrated to aux-disk on LXC211, LXC230, LXC200, LXC220: containerd + Docker data moved from local-lvm root disks to existing aux-disk mounts; LXC220 received new `mp1` (`/mnt/aux-disk/calibreweb`); SSD thin-pool reduced from 88% → 62% (24.9 GB freed after fstrim) |
