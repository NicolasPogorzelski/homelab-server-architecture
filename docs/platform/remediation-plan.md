...
| 9 | Fold the hand-deployed host units into roles | `node_exporter`, `wait-for-tailscale-ip.sh`, `lxc-fstrim`, `lvm-thin-metrics` — all lost on a rebuild. **Add `/etc/snapraid.conf` on vm102 (2026-08-15):** the `snapraid_maintenance` role owns the script and both timers but not the configuration, so the [KE-19](known-errors.md#ke-19) exclude rules are hand-made and would not survive a rebuild. vm102 *is* an Ansible node, so unlike the host items this one is not blocked on anything |
