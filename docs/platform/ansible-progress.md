...
- **`breakglass` role complete (2026-06-08):**
  ...
  - `inventory/group_vars/vms.yml` - `breakglass_pubkeys` (the `admin-workstation` + `admin-laptop` admin pubkeys; public keys, not Vault. A legacy workstation key was replaced by the current `admin-workstation` key on 2026-06-17)
...
- **Session 2026-06-12d (Item #13 - CI/CD + ansible-lint - DONE):** built `.github/workflows/ansible-lint.yml` - lightweight GitHub Actions pipeline. Triggers: `push` (all branches) + `pull_request` (branches: main). Single job `Ansible_lint` on `ubuntu-latest`: (1) `actions/checkout@v6`; (2) `pip install ansible-lint` + `ansible-galaxy collection install -r ansible/requirements.yml` (new file - lists `ansible.posix`, `community.postgresql`, `community.docker`); (3) `cd ansible && ansible-lint .` (must run from `ansible/` so `ansible.cfg` `roles_path = ./roles` resolves correctly). First run surfaced 61 violations in two categories: (A) structural - roles not found (wrong search path) + missing collections; (B) code quality - `name[casing]` (27x), `yaml[truthy]` (7x: `yes`/`no` -> `true`/`false`), `risky-shell-pipe` (4x: `set -o pipefail` + `executable: /bin/bash` added), `no-changed-when` (2x on shell/command handlers: `changed_when: true`), `risky-file-permissions` (missing `mode:` on `get_url`), `yaml[indentation]`, `partial-become` (`become: true` missing alongside `become_user`), `command-instead-of-module` (suppressed via `# noqa:` - no ansible module equivalent for `apt-get -s dist-upgrade`). All fixed; second run pipeline green. Key learning: `notify:` values must match handler names exactly (case-sensitive string match) - renaming a handler without updating `notify:` silently breaks the handler without any error. Ansible track complete.

- **Session 2026-06-11b (tooling/meta):** no platform change. The session covered the local
  editing setup and produced no Ansible work. Two ideas were parked, recorded here only so they
  are not rediscovered as new: a shell-out pattern for output-heavy boilerplate across five or
  more near-identical files, and a small Model Context Protocol server (~50 lines of Python)
  wrapping Ollama, whose value is the learning payoff rather than the tool itself. Neither is
  decided. The next Ansible task was unchanged by this session: Roadmap Item #11, the PostgreSQL
  backup playbook.
