...
| `systemd_hygiene` | `all` | Per-host list of units that sit permanently in `failed` and drown out real faults now that `SystemdUnitFailed` alerts on unit state. Each entry names a `reason` and an `action`: `mask` for package-owned units (lxc210 `run-rpc_pipefs.mount`, lxc260 `nvmf-autoconnect`/`openipmi`), `remove` for hand-written ones (vm100's legacy `media-mergerfs.mount`) — `systemctl mask` refuses to overwrite an existing unit file. Finishes with `reset-failed` |
