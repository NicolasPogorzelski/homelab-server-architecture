...
| `homelab_schedule` | Proxmox host (`proxmox`) | Deploys `homelab-setwake.sh` + `homelab-shutdown.sh` to `/usr/local/sbin/` and manages `/etc/cron.d/homelab-schedule` via template. Source scripts: `scripts/homelab-setwake.sh` + `scripts/homelab-shutdown.sh`. Note: Proxmox host uses `ansible_user: root` — no `ansible` user bootstrapped there. |
