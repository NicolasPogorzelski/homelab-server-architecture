...
| `journal_remote` | LXC200 | **Exercise** ([decision](../decisions/exercise-scope-before-terraform.md)). Receives journals from the nodes that opt in, using systemd's own remoting rather than Loki or an ELK stack: journald already holds structured records with verified boot ids, which a line-based collector would discard and a parser would guess back. Bound to the Tailscale address and the packaged wildcard socket masked, since leaving it enabled beside a pinned service means systemd activates the socket on first connection and the bind is bypassed. Received journals are capped at 2 GB because this container's root filesystem is thin-pool space on the boot SSD |
