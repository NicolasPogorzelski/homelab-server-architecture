...
| A.8.5 Secure authentication | Partial | Re-measured 2026-09-04 with `sshd -T` as root on every inventory node, which is the only reading that resolves the `Include` order. `passwordauthentication no` and `kbdinteractiveauthentication no` hold on all ten; `permitrootlogin no` on nine, `without-password` on the Proxmox host by the exception in A.8.2. The effective state is therefore correct everywhere and the mechanism behind it is not uniform. The `ssh_hardening` role writes two things: a `00-hardening.conf` drop-in, which wins over anything the distribution later places in `sshd_config.d`, and lines in `sshd_config`, which lose against it. The drop-in exists on vm100, lxc250 and the Proxmox host only; on lxc200, lxc210, lxc211, lxc220, lxc230, lxc260 and vm102 the directory is empty, so the setting rests on the weaker of the two. The cause is `6faf809` (2026-07-08), which added the drop-in task to fix a cloud-init collision on vm100 and was applied there; the role has not run fleet-wide since. Applying it is a scheduled maintenance unit, not a live gap - vm100 is the only node with a competing file, and 00- sorts before 50-. Two earlier claims in this row were wrong and are struck: that the host and lxc250 carry a hand-written file no role maintains (both have been role-managed since 2026-08-20 and 2026-08-21), and the count of eleven nodes (ten since lxc240 was withdrawn). This row read *Enforced* until 2026-08-17 while both nodes accepted passwords - the wrong status is the finding, because a false assurance suppresses discovery more effectively than a stated gap does, and this correction is the same lesson applied to the correction itself. |
