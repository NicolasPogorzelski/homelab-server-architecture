...
- Two inference backends are planned (admin workstation + VM100).
...
| App state / config | Medium | Local (aux-disk) | Stable, fast, reboot-safe |
| Logs | Low | Local (aux-disk) | Avoids network noise |
...
- Local via bind mount (planned): `mp1: /mnt/aux-disk/openwebui -> /var/lib/openwebui`

---

## Planned Volume Layout (CT230)

- App data: `/var/lib/openwebui/appdata` (aux-disk)
- Logs: `/var/lib/openwebui/logs` (aux-disk)
