...
- Installation: `ollama-rocm` via pacman (admin workstation repository)
- Override: `/etc/systemd/system/ollama.service.d/override.conf`
...
- admin workstation: rocBLAS probe-runner crashes on startup (non-blocking, known ROCm/gfx1100 issue)
...
If the admin workstation (primary) becomes unavailable:
...
- admin workstation continues as primary; no user-visible impact
- No data loss
